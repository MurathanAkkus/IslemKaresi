...
{                                   //...Bu sayede; Start,FixUpdate,Update gibi metotlar kullanilabilinir.
    ...
    //    }
    //}
    private void GridDebugLog(string[] str)
    {
        for(int i = 0; i < 36; i++)
        {
            Debug.Log(str[i]);
    ...
    private void SetGridSquareData()
    {
        ...
        GridDebugLog(solved.ToArray());
        unsolved = SetData(solved, unsolved, "Medium");
        foreach (var square in grid_squares_)
        {
            int index = grid_squares_.IndexOf(square);
        ...
        for (int j = 0; j < 36; j++)
        {
            if (j != 5 || j != 17 || j != 29 || j != 30 || j != 32 || j != 34
                ...
                if (Int32.TryParse(solved[j], out out_))
                    if(Int32.Parse(solved[j]) >= 1 && Int32.Parse(solved[j]) <= 9)
                        unsolved[j] = null;
            Debug.Log(unsolved[j]);
        }

...
{                                   //...Bu sayede; Start,FixUpdate,Update gibi metotlar kullanilabilinir.
    ...
    public Color opt_col = new Color32(200, 227, 208, 200);
    public Color res_col = new Color32(200, 200, 200, 200);
    ...
    private void SetAllSqaureColor()
    {
        ...
        foreach (var square in grid_squares_)
        {
            ...
            if (comp.IsOpt_Square())
                comp.SetSquareColor(opt_col);
            else if (i == 5 || i == 17 || i == 29 || i == 30 || i == 32 || i == 34)
            {
    ...
    private void OnEnable()
    {
        ...
        GameEvents.OnCheckNumberControl += CheckNumberControl;
    }
    ...
    public void OnSquareSelected(int square_index)
    {
        ...
        if (!grid_squares_[square_index].GetComponent<GridSquare>().GetHasDefaultValue())
        {
            ...
            if (!comp.IsOpt_Square() && !comp.HasWrongValue() && !comp.GetHasDefaultValue())
        ...
        else
        {
            foreach (var gridSquare in grid_squares_)
            {
                ...
                if (!comp.HasWrongValue() && !comp.IsOpt_Square()) //&& !comp.IsSelected()
                    comp.SetSquareColor(Color.white);
    ...
    public void CheckNumberControl()
    {
        foreach (var square in grid_squares_)
        {
            var comp = square.GetComponent<GridSquare>();
            if (comp.IsCorrectNumberSet() && !comp.IsOpt_Square() && !comp.IsRes_Square(res_col))
                GameEvents.NumberButtonControlMethod();
        }
    }

    //private void NumberButtonControl(GameObject number_button)
    //{
    //    GridSquare comp;
    //    foreach (var square in grid_squares_)
    //    {
    //        comp = square.GetComponent<GridSquare>();

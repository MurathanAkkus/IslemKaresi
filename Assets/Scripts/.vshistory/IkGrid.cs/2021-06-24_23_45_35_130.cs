...
{                                   //...Bu sayede; Start,FixUpdate,Update gibi metotlar kullanilabilinir.
    private int columns = 0; // sutun
    private int rows = 0;    // satir
    private float square_offset = 0.0f; // karelerin pozisyonunu offset kadar kaydiririz

    private Vector2 start_position = new Vector2(0.0f, 0.0f);  // baslangic pozisyonu
    private float square_scale = 1.0f; // kare olcegi (boyut degildir, boyut = scale(x,y) * W,H)

    private static List<GameObject> grid_squares_ = new List<GameObject>(); // Grid'in karelerinin listesi
    ...
    //        square.GetComponent<GridSquare>().SetHasDefaultValue(data.unsolved_data[index] != "-2" && data.unsolved_data[index] == data.solved_data[index]);
    //    }
    //}

    private void SetGridSquareData()
    {
        foreach (var square in grid_squares_)
        {
            int index = grid_squares_.IndexOf(square);
            square.GetComponent<GridSquare>().SetNumber(unsolved[index], square);
        }
    }

    private static string str, num_opt;

    private static List<string> SetGridValues()
    {
    ...
    private static List<string> solved = SetGridValues();
    private static string[] unsolved = SetData(solved, unsolved, "easy");

    private static string[] SetData(List<string> solved, string[] unsolved, string mode)
    {
        for (int j = 0; j < 29; j++)
        {
            if (j == 5 || j == 17)

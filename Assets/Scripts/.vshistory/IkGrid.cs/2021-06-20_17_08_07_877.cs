    ...
    public List<string> SetGridValues()
    {
        ...
        foreach (var square in grid_squares_)
        {
            if (column_number == 5 && row_number != 5)
            {
                ...
                {
                    num_opt = square.GetComponent<Results>().Result(Trans(x_array, row_number));
                    str = "";
                }
                else
                    num_opt = "-1";
            ...
            else if (row_number == 5)
            {
                ...
                {
                    num_opt = square.GetComponent<Results>().Result(Trans(y_array, column_number));
                    str = "";
                }
            ...
            if (square.active == true)
            {
                ...
                Debug.Log("deneme_sonlarda");
            }
            column_number++;
            solved.Add(num_opt);
            Debug.Log(num_opt);
            Debug.Log("deneme_son");
            Debug.Log(solved[0]);
        }
        return solved;
    }

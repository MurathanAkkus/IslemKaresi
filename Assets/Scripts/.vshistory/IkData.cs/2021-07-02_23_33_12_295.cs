...
public class IkData : MonoBehaviour
{
    ...
    public static string[] SetData(List<string> solved, string mode)
    {
        ...
        if (i != 0)
        {
            do
            {
                do { }while(numbers.ToArray().IndexOfAny(rand.ToString()) < 0)
                rand = Random.Range(0, numbers.Count - 1);
                
                for(int j = 0; j < 28; j++)
                    // rastgele üretilen index'teki sayinin
                    if (solved[j].Equals(rand) && unsolved[j].Equals(" "))
                        idx = j;    // tablodaki yerini bulup, onun index'ini aliyor.
                numbers.Remove(rand.ToString());
                Debug.Log(idx + "\t" +rand);

        ...
        public IkBoardData(List<string> solved, string mode)
        {
            ...
            if(i != 0)
            {
                do
                {
                    ...
                    index = solved.IndexOf(numbers[rand]);  // rastgele üretilen index'teki sayinin 
                    numbers.RemoveAt(rand); // tablodaki yerini bulup, onun index'ini aliyor.
                    unsolved_data[index] = solved[index]; // Oradaki index'teki sayiyi aciyor kullaniciya
                    i--;
                } while (i > 0);
            }
        }
